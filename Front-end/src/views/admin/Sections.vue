<script setup lang="ts">
import { ref, onMounted, computed } from "vue";
import { storeToRefs } from "pinia";
import DataTable from "../../components/DataTable.vue";
import EntityForm from "../../components/EntityForm.vue";
import ConfirmDialog from "../../components/ConfirmDialog.vue";
import type { DataTableHeader, FormField } from "../../types/types.ts";
import { useSectionStore, type SectionPayload } from "../../stores/sections";
import { Section } from "../../types/types.ts";
const sectionStore = useSectionStore();
const { sections, loading, error } = storeToRefs(sectionStore);

const showFormDialog = ref(false);
const showConfirmDialog = ref(false);
const isEditing = ref(false);

const defaultItem: Section = { id: 0, libelle: '', nbrEtudiants: 0 };
const editedItem = ref<Section>({ ...defaultItem });
const itemToDeleteId = ref<number | null>(null);

const headers: DataTableHeader[] = [
  { key: "libelle", title: "Libellé de la Section" },
  { key: "nbrEtudiants", title: "Nombre d'étudiants" },
  { key: "actions", title: "Actions", sortable: false, align: 'end' },
];

const formFields: FormField[] = [
  { key: "libelle", label: "Libellé", type: "text", required: true },
  { key: "nbrEtudiants", label: "Nombre d'étudiants", type: "number", required: true },
];

const formTitle = computed(() => isEditing.value ? "Modifier la section" : "Ajouter une section");

onMounted(() => {
  sectionStore.fetchSections().catch(err => console.error("Erreur au montage:", err.message));
});

function openAddForm() {
  isEditing.value = false;
  editedItem.value = { ...defaultItem };
  showFormDialog.value = true;
}

function openEditForm(item: Section) {
  isEditing.value = true;
  editedItem.value = { ...item };
  showFormDialog.value = true;
}

function openDeleteConfirm(item: Section) {
  itemToDeleteId.value = item.id;
  showConfirmDialog.value = true;
}

function closeFormDialog() { showFormDialog.value = false; }
function closeConfirmDialog() { showConfirmDialog.value = false; itemToDeleteId.value = null; }

async function saveForm() {
  try {
    if (isEditing.value) {
      await sectionStore.updateSection(editedItem.value);
    } else {
      const payload: SectionPayload = {
        libelle: editedItem.value.libelle,
        nbrEtudiants: Number(editedItem.value.nbrEtudiants)
      };
      await sectionStore.addSection(payload);
    }
    closeFormDialog();
  } catch (err) {
    console.error("Échec de la sauvegarde:", err);
  }
}

async function confirmDelete() {
  if (itemToDeleteId.value !== null) {
    try {
      await sectionStore.deleteSection(itemToDeleteId.value);
      closeConfirmDialog();
    } catch (err) {
      console.error("Échec de la suppression:", err);
    }
  }
}
</script>

<template>
  <div>
    <h1 class="text-h4 font-weight-bold mb-6">Gestion des Sections</h1>
    <v-alert v-if="error" type="error" closable class="mb-4">{{ error }}</v-alert>

    <DataTable
        title="Liste des sections"
        :headers="headers"
        :items="sections"
        :loading="loading"
        @add="openAddForm"
        @edit="openEditForm"
        @delete="openDeleteConfirm"
    />

    <v-dialog v-model="showFormDialog" max-width="600px" persistent>
      <EntityForm
          v-model="editedItem"
          :fields="formFields"
          :title="formTitle"
          :is-edit="isEditing"
          @submit="saveForm"
          @cancel="closeFormDialog"
      />
    </v-dialog>

    <ConfirmDialog
        v-model="showConfirmDialog"
        title="Supprimer la section"
        text="Êtes-vous sûr de vouloir supprimer cette section ? Cette action est irréversible."
        confirm-text="Supprimer"
        @confirm="confirmDelete"
        @cancel="closeConfirmDialog"
    />
  </div>
</template>